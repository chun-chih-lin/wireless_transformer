...
# ==========================================================
def get_packets(ary, pkt_size=500, mov_wdw_s=250):
    abs_ary = np.abs(ary)
    n_ary = ary.shape[0]
    ...
    mov_avg_threshold = (np.max(mov_avg) + np.mean(mov_avg)*2)/3
    mov_avg_threshold = np.mean(mov_avg)
    ...
    if INSPECT:
        ...
        plt.axhline(np.max(mov_avg), color='k', linewidth=0.5)
        plt.axhline(np.mean(mov_avg), color='k', linewidth=0.5)
