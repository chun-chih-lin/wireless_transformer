...
ROOM328_PREFIX="Dataset_EIB_room328_to_Hallway_0521"
HALLWAY_PREFIX="Dataset_EIB_3F_Hallway_0521"
LINUX=0
WINDOWS=1
...
def extract_features(data):
    ...
    save_pkl_folder = os.path.join(args.source_folder, args.pattern)
    if not os.path.exists(save_pkl_folder):
        print(f"Folder: {save_pkl_folder} does not exist. Create a new one")
...
def get_data(args):
    ...
    full_filename = os.path.join(args.source_folder, filename)
...
def main():
    ...
    t2 = process_time()
    print(f"Total using time: {t2 - t1}")
    pass
